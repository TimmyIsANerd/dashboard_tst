...
getTransactions = async () => {
    // The transactions are hardcoded to retrieve only bsc transactions. 
    ...
    if (transactions.total > 0) {
        ...
        transactions.result.forEach(t => {
            let content = `
            <tr>                
                <td><a href='https://bscscan.com//tx/${t.hash}' target="_blank" rel="noopener noreferrer">${t.hash}</a></td>
                <td><a href='https://bscscan.com/block/${t.block_number}' target="_blank" rel="noopener noreferrer">${t.block_number}</a></td>
                <td>${millisecondsToTime(Date.parse(new Date()) - Date.parse(t.block_timestamp))}</td>
                <td>${t.from_address == Moralis.User.current().get('ethAddress') ? 'OUT' : 'IN'}</td>
...
getERC20Balances = async () => {
    ...
    if(bscTokens.length > 0){
        ...
        bscTokens.forEach((e,i) => {
                ...
                <td>${e.decimals}</td>                
                <td><a href='https://bscscan.com/token/${e.token_address}' target="_blank" rel="noopener noreferrer">${e.token_address}</a></td>
                </tr>
...
getNFTs = async () => {
        ...
        if (_chain == 'none'){return alert('Choose a chain from the dropdown list');}
